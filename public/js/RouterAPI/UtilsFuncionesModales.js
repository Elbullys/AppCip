...
export class BuscadorGenericoSelectFiltro {
  ...
  abrir(campo,nombre) {
    ...
    this.listaResultados.innerHTML = '<div class="text-center p-3 text-muted">Escribe algo para buscar</div>';
...
if (config.getExtraParams && typeof config.getExtraParams === 'function') {
    ...
    Object.keys(extras).forEach(key => {
      if (extras[key]) { // Solo agrega si el valor no es null/undefined
  ...
  try {
    ...
    const items = Array.isArray(data) ? data : (data.body && Array.isArray(data.body) ? data.body : []);
    
    if (!Array.isArray(items)) {
      console.error("La respuesta no contiene un array válido:", items);
      this.listaResultados.innerHTML = '<div class="text-center p-3 text-danger">Respuesta inválida del servidor</div>';
      return [];
    }
    ...
    this.listaResultados.innerHTML = '<div class="text-center p-3 text-danger">Error al buscar: ' + error.message + '</div>';
    return [];
  }
}
...
async ejecutarBusqueda() {
    ...
    if (items.length === 0) {
        this.listaResultados.innerHTML = '<div class="text-center p-3 text-muted">No hay resultados</div>';
        return;
    }
